...
def solve(instance: ProblemInstance, config: dict | None = None) -> Schedule:
    ...
    base_order = _pick_balanced_order(instance)
    order, memory, spill_entries = _assign_memory_with_spills(instance, base_order)
    schedule.order = order
    schedule.memory = memory
    schedule.spill_entries = spill_entries
    return schedule
...
def _pick_balanced_order(instance: ProblemInstance) -> list[int]:
    nodes = {node.id: node for node in instance.nodes}
    candidates = [
        _memory_aware_order(instance, variant="normalized"),
        _memory_aware_order(instance, variant="generic"),
        _memory_aware_order(instance, variant="p1"),
    ]
    return min(candidates, key=lambda order: _overflow_traffic(order, nodes))
...
def _memory_aware_order(instance: ProblemInstance, variant: str) -> list[int]:
    ...
    if variant == "normalized":
        use_id_alloc_tiebreak = any(node.op == "D2S" for node in instance.nodes)
    weight = NORM_CACHE_WEIGHT if variant == "normalized" else P1_CACHE_WEIGHT
    ...
    while ready_free or ready_ops or ready_alloc:
        if ready_free:
            ...
            while alloc_heap:
                *priority, candidate = heapq.heappop(alloc_heap)
                if candidate in ready_alloc and tuple(priority) == alloc_priority(nodes[candidate])[:-1]:
                    node_id = candidate
...
class _FreeSpace:
    ...
    def find(self, size: int) -> int | None:
        ...
        for offset, free_size in self.free:
            if free_size >= size and (best is None or free_size < best[1]):
                best = (offset, free_size)
...
def _assign_memory_with_spills(
    ...
    def make_room(mem_type: str, size: int, pos: int, pinned: set[int]) -> int:
        ...
        while offset is None:
            ...
            victim = max(victims, key=lambda b: (next_event(bufs[b], pos), bufs[b].size))
            spill_out(victim)
            offset = space[mem_type].find(size)
        return offset
