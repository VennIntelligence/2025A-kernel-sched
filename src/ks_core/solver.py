...
* ``scripts/paper/harness.py`` imports this module to drive the paper experiments,
  exercising ``victim_policy`` and the other internal helpers directly.
        ...
        while offset is None:
            ...
            if victims:
                ...
                def victim_score(b: int) -> tuple:
                    ...
                    cost = bufs[b].size if is_clean else 2 * bufs[b].size
                    size = bufs[b].size
                    if victim_policy == "dist_size_cost":
                    ...
                    if victim_policy == "far_only":
                        return (dist, size)
                    if victim_policy == "size_only":
                        return (size, dist)
                    raise ValueError(f"unknown victim_policy {victim_policy}")
